...
docker compose up -d --wait              # data plane + watchdog + confidence + mcp-domain + finance + policy (portero) + bridge + pwa (+ openclaw si hay API key)
...
- Verificación automática: `pnpm test` en `sim/`, `router/`, `services/watchdog/`, `services/confidence/`, `services/bridge/`, `services/mcp-domain/`, `services/finance/`, `services/policy/`, `pwa/`; `cd sim && pnpm contract` (mensajes vivos vs AsyncAPI v0.6.0, incluye plano plataforma 4-seg y cmd); E2E Fase 3 con stack vivo: `pnpm --dir e2e ec-baja` (escenario "EC baja" de punta a punta).
