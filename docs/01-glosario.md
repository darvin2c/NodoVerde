...
| **Módulo** | Unidad lógica de asignación: el nombre de un lugar físico (hoy: mesa hidropónica). Es infraestructura **fungible** (ADR-0025): NO tiene cultivo propio — el cultivo lo pone el lote que lo ocupa y se retira al cosechar. Tiene `id` técnico autogenerado (mod-N) y `name` humano libre ("Mesa Norte"). Se retira, jamás se borra (ADR-0022) | La mesa de cultivo con nombre en la puerta: hoy aloja lechuga, mañana tomate — la mesa es la misma, la vida cambia |
| **Finca (tenant)** | Unidad de operación independiente: `id` slug inmutable (queda grabado en topics MQTT y ledger), `name` humano, lat/lon obligatorias (de ellas salen el clima y la zona horaria derivada en código) y `currency` propia. Se crea/edita/archiva desde la PWA vía MCP gobernado; archivar conserva toda la historia — nada se borra (ADR-0023). Las sumas financieras jamás mezclan monedas de fincas distintas | La parcela con su propio libro de cuentas: cada una habla su moneda y guarda su historia, aunque un mismo dueño mire todas juntas |
